...
class Subject{
	...
	protected static $db_fields = array('sub_id','sub_name', 'year', 'sem_id','course','staff_id','credits');
  ...
  public function allocate_lec(){
	global $database;
		if(mysqli_num_rows($database->query("SELECT * FROM users WHERE staff_id='{$this->staff_id}' AND privilege='lecturer' "))!=0){
			if(mysqli_num_rows($database->query("SELECT * FROM lecturer_subject WHERE staff_id='{$this->staff_id}' AND  sub_id='{$this->sub_id}' "))==0){
			...
			echo '<script language="javascript">';
				echo 'alert("The lecturer has been successfully allocated!")';
				echo '</script>';
			}else{
				echo '<script language="javascript">';
				echo 'alert("The lecturer has been already allocated!")';
				echo '</script>';
			}
		}else{
			echo '<script language="javascript">';
			echo 'alert("error! The staff id is not in the system!")';
			echo '</script>';			
		}
